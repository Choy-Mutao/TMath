...
namespace RTree
{
    ...
    public class RTree<T>
    {
        ...
        public void Add(TPoint2D p1, TPoint2D p2, T item)
        {
            Rectangle r = new Rectangle(p1.X, p1.Y, 0, p2.X, p2.Y, 0);
            Add(r, item);
        }

        public void Add(TLineSegment2d segment, T item)
        {
            Add(segment.SP, segment.EP, item);
        }
        ...
        private void Nearest(Point p, Action<int> v, double furthestDistance)
        {
        ...
        /// <returns></returns>
        private double nearest(Point p, Node<T> n, List<int> nearestIds, double nearestDistance)
        {
            for (int i = 0; i < n.entryCount; i++)
            {
